...
# Preprocessors ;
   ...
   if ($pre ne "") {
      ...
      foreach $thisp (@tempo) {
         ...
         require $package .".pm" ;
	print STDERR "Exec Pre : $package::$sname\n" ;
         &$thisp( $rh);
      }
   }
# Proccessors 
   ...
   if ($proc ne "") {
	print STDERR "Procs $proc\n"  ;
      @tempo = split /\s+/, $proc;
      foreach $thisp (@tempo) {
         next unless $thisp =~ /\w+/ ;
	 ($package, $sname) = split /::/,$thisp;
	 require $package . ".pm" ;
	print STDERR "Exec Proc: $thisp\n" ;
         &$thisp( $rh );
      }
   }
...
# Postproccessors 
   ...
   if ($post ne "") {
      ...
      foreach $thisp (@tempo) {
         ...
         ($package, $sname) = split /::/,$thisp;
         require $package.".pm";
	print STDERR "Exec proc: $package::$sname\n" ;
         &$thisp( $rh );
      }
   }
...
#	print STDERR "fichero=\n$string\n" ;
	$string =~  /.*<title>(.*)<\/title>/si ;
	my $title =$1 ;
	$string =~/.*<body.*>(.*)<\/body>/si ;
	my $wbbin =$1 ;
	$webberhash{'title'} = $title ;
	$webberhash{'wbbIn'} = $wbbin ;

#	print STDERR "title=$title\n" ;
#	print STDERR "wbbIn=$wbbin\n" ;
	print STDERR "Antes de Webbear\n" ;
#	foreach my $k (keys %webberhash) { print STDERR "WEBBER $k ==> $webberhash{$k}\n" ; }
